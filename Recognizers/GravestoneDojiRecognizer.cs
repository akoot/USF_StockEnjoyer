    ...
    internal class GravestoneDojiRecognizer : PatternRecognizer
    {
        public override void Draw(Chart chart)
        {
            ...
            for (int i = 0; i < candles.Count - 1; i++)
            {
                ...
                bool isGravestone = current.Open == current.High && current.Close == current.Low && Math.Abs(current.Open - current.Close) < 0.01 * current.High;
                ...
                if (isGravestone && isFollowingCandle)
                {
                    chart.Series[0].Points[i + 1].Color = Color.AliceBlue;
                }
            }
        }
    }
